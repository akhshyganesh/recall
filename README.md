...
- Supports:
  ...
  - startup update checks against the latest signed GitHub Release on production builds
...
- When a newer signed release exists, the sidebar shows an **Update available** action above Settings and the Settings view exposes manual check/install controls.
...
- In-app updates currently ship for the platforms built by the release workflow: **macOS arm64**, **macOS x64**, and **Linux x64**.
...
The GitHub `production` environment used by the release workflow must also define these updater signing secrets:

- `TAURI_SIGNING_PRIVATE_KEY`: the full minisign private key generated by `npm run tauri signer generate -- -w ~/.tauri/recall.key`, including the `untrusted comment:` header. The workflow also accepts a base64-encoded copy of the whole key file.
- `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`: the password for that private key, if you set one when generating it.

If the key secret is missing the `untrusted comment:` header or only contains the encoded body line, updater artifact signing will fail during the release build.
