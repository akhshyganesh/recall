...
fn supported_agent_filter(column: &str) -> String {
    let slugs = SUPPORTED_AGENT_SLUGS
        .iter()
        .map(|slug| format!("'{slug}'"))
        .collect::<Vec<_>>()
        .join(", ");
    format!("{column} IN ({slugs})")
}
...
fn append_path_scope_filter(
    sql: &mut String,
    session_path_expression: &str,
    source_path_expression: &str,
    paths: Option<&[String]>,
    param_values: &mut Vec<Box<dyn rusqlite::types::ToSql>>,
) {
    let Some(paths) = paths.filter(|current_paths| !current_paths.is_empty()) else {
        return;
    };
    ...
    for path in paths {
        ...
        param_values.push(Box::new(normalized.clone()));
        let exact_index = param_values.len();
        param_values.push(Box::new(format!("{normalized}/%")));
        let child_index = param_values.len();

        clauses.push(format!(
            "(REPLACE({session_path_expression}, '\\', '/') = ?{exact_index} OR REPLACE({session_path_expression}, '\\', '/') LIKE ?{child_index} OR REPLACE({source_path_expression}, '\\', '/') = ?{exact_index} OR REPLACE({source_path_expression}, '\\', '/') LIKE ?{child_index})"
        ));
    }
...
impl Database {
    ...
    pub fn get_session_summaries(
        ...
        sql.push_str(&format!(" AND {}", supported_agent_filter("s.agent_slug")));
    ...
    pub fn get_sessions_for_agent(
        ...
        let rows = stmt
            .query_map(params![agent_slug, limit as i64, offset as i64], |row| {
                Ok(SessionSummary {
                    id: row.get(0)?,
                    tool: row.get(1)?,
                    agent_slug: row.get(2)?,
    ...
    pub fn get_session(&self, id: &str) -> Result<Option<Session>, String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        let mut stmt = conn.prepare(&format!(
            "SELECT id, tool, agent_slug, source_path, repo_name, repo_path, branch, title, started_at, ended_at, model, message_count, file_count, workspace, external_id, metadata, indexed_at, source_mtime FROM sessions WHERE id = ?1 AND {}",
    ...
    pub fn get_tools(&self) -> Result<Vec<String>, String> {
        ...
        let mut stmt = conn
            .prepare(&format!(
                "SELECT DISTINCT tool FROM sessions WHERE {} ORDER BY tool",
                supported_agent_filter("agent_slug")
            ))
            .map_err(|e| e.to_string())?;
        let rows = stmt
    ...
    pub fn get_stats(&self, path_filters: Option<&[String]>) -> Result<Stats, String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        let mut where_sql = format!("WHERE {}", supported_agent_filter("s.agent_slug"));
        let mut param_values: Vec<Box<dyn rusqlite::types::ToSql>> = Vec::new();
        append_path_scope_filter(
            &mut where_sql,
            SESSION_PATH_EXPR_WITH_ALIAS,
        ...
        let total_messages: i64 = conn
            .query_row(
                &format!("SELECT COUNT(*) FROM messages m JOIN sessions s ON m.session_id = s.id {where_sql}"),
                params_ref.as_slice(),
                |r| r.get(0),
            )
            .map_err(|e| e.to_string())?;
        let total_tools: i64 = conn
            .query_row(
                &format!("SELECT COUNT(DISTINCT s.tool) FROM sessions s {where_sql}"),
                params_ref.as_slice(),
                |r| r.get(0),
            )
            .map_err(|e| e.to_string())?;
    ...
    pub fn get_activity_heatmap(
        ...
        let mut sql = format!(
            "SELECT substr(COALESCE(s.started_at, s.indexed_at), 1, 10) AS activity_date,
                        s.tool,
                        COUNT(*) AS session_count
                 FROM sessions s
                 WHERE substr(COALESCE(s.started_at, s.indexed_at), 1, 10) >= ?1
                 AND {}
                 ",
            supported_agent_filter("s.agent_slug")
        );
        ...
        sql.push_str(" GROUP BY activity_date, s.tool ORDER BY activity_date ASC, session_count DESC, s.tool ASC");
        let params_ref: Vec<&dyn rusqlite::types::ToSql> =
            param_values.iter().map(|p| p.as_ref()).collect();
