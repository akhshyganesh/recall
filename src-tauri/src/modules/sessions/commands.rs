...
#[tauri::command]
pub async fn get_sessions_mcp_status(state: State<'_, AppState>) -> AppResult<McpStatus> {
    Ok(state.mcp.status().await)
}

#[tauri::command]
pub async fn set_sessions_mcp_enabled(
    state: State<'_, AppState>,
    enabled: bool,
) -> AppResult<McpStatus> {
    state.mcp.set_enabled(state.db_handle(), enabled).await
}
...
#[tauri::command]
pub fn get_stats(state: State<AppState>, paths: Option<Vec<String>>) -> AppResult<Stats> {
    with_db(&state, |db| db.get_stats(paths.as_deref()))
}
...
pub fn get_activity_heatmap(
    ...
    with_db(&state, |db| {
        db.get_activity_heatmap(days.unwrap_or(182), paths.as_deref())
    })
}
