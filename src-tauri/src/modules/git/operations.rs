...
pub fn list_branches(
    ...
    repo_root: &str,
    workspace: &WorkspaceEnv,
) -> Result<GitBranchSnapshot> {
    let repo_root = authorized_repo_root(registry, repo_root, workspace)?;
    ensure_git_available(&repo_root.workspace)?;
    list_branches_inner(&repo_root)
}

fn list_branches_inner(repo_root: &ResolvedGitDirectory) -> Result<GitBranchSnapshot> {
    let local_lines = git_stdout_lines(
        &repo_root.workspace,
        &repo_root.git_path,
        [
            "for-each-ref",
    ...
    let mut local_branches: Vec<GitLocalBranch> = local_lines
        .into_iter()
    ...
    let mut remote_branches: Vec<GitRemoteBranch> = remote_lines
        .into_iter()
...
fn parse_local_branch_line(line: String) -> Option<GitLocalBranch> {
...
fn parse_remote_branch_line(line: String) -> Option<GitRemoteBranch> {
    let name = line.trim();
    if name.is_empty() || name.ends_with("/HEAD") {
        return None;
    }
    let (remote, short_name) = name.split_once('/')?;
    if remote.is_empty() || short_name.is_empty() {
        return None;
    }
    Some(GitRemoteBranch {
        name: name.to_string(),
...
pub fn switch_branch(
    ...
    let repo_root = authorized_repo_root(registry, repo_root, workspace)?;
    ensure_git_available(&repo_root.workspace)?;
    let branch = validate_branch_name(&repo_root, name, "git switch failed")?;
    let output = run_git(
        &repo_root.workspace,
        Some(&repo_root.git_path),
        [OsStr::new("switch"), OsStr::new(&branch)],
        DEFAULT_TIMEOUT_SECS,
    )?;
    ensure_success(&output, "git switch failed")?;
    Ok(GitBranchMutationResult { branch })
}
...
pub fn switch_remote_branch(
    ...
    let repo_root = authorized_repo_root(registry, repo_root, workspace)?;
    ensure_git_available(&repo_root.workspace)?;
    let remote_branch = validate_remote_branch_name(&repo_root, name)?;
    let (_, local_branch) = remote_branch.split_once('/').ok_or_else(|| {
        GitError::command(
            "git switch failed",
            "remote branch must be in the form <remote>/<branch>",
        )
    })?;

    let local_ref = format!("refs/heads/{local_branch}");
    let local_exists = git_ref_exists(&repo_root, &local_ref)?;
    let output = if local_exists {
        run_git(
            &repo_root.workspace,
            Some(&repo_root.git_path),
            [OsStr::new("switch"), OsStr::new(local_branch)],
            DEFAULT_TIMEOUT_SECS,
        )?
    } else {
        run_git(
...
pub fn create_branch(
    ...
    if let Some(start) = start_point.filter(|value| !value.trim().is_empty()) {
        ...
        ensure_success(&output, "git branch creation failed")?;
        return Ok(GitBranchMutationResult { branch });
    }
    let output = run_git(
        &repo_root.workspace,
        Some(&repo_root.git_path),
...
fn validate_branch_name(
    ...
    if trimmed.is_empty() {
        return Err(GitError::command(context, "branch name cannot be empty"));
    }
    let output = run_git(
        &repo_root.workspace,
        Some(&repo_root.git_path),
...
fn validate_remote_branch_name(repo_root: &ResolvedGitDirectory, name: &str) -> Result<String> {
    ...
    if !git_ref_exists(repo_root, &full_ref)? {
        return Err(GitError::command(
            "git switch failed",
            format!("remote branch not found: {trimmed}"),
        ));
    }
    Ok(trimmed.to_string())
}

fn validate_start_point(repo_root: &ResolvedGitDirectory, start_point: &str) -> Result<String> {
    let trimmed = start_point.trim();
    if trimmed.is_empty() {
        return Err(GitError::command(
            "git branch creation failed",
            "branch start point cannot be empty",
        ));
    }
    if let Some((remote, short_name)) = trimmed.split_once('/') {
        if !remote.is_empty() && !short_name.is_empty() {
            validate_branch_name(repo_root, short_name, "git branch creation failed")?;
            let full_ref = format!("refs/remotes/{trimmed}");
            if git_ref_exists(repo_root, &full_ref)? {
    ...
    let branch = validate_branch_name(repo_root, trimmed, "git branch creation failed")?;
...
fn git_ref_exists(repo_root: &ResolvedGitDirectory, reference: &str) -> Result<bool> {
    let output = run_git(
        &repo_root.workspace,
        Some(&repo_root.git_path),
        [
