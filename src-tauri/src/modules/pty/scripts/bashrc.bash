# recall-shell-integration (bashrc)
#
...
if [ -z "$__RECALL_HOOKS_LOADED" ]; then
  ...
  _recall_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_recall_urlencode "$PWD")"
    if [ -z "$__RECALL_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __RECALL_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
