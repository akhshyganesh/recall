# recall-shell-integration (zshrc)
#
...
# zsh, so we shadow $? into `_recall_ret`.

{
  _recall_user_zdotdir="${RECALL_USER_ZDOTDIR:-$HOME}"
  [ -f "$_recall_user_zdotdir/.zshrc" ] && source "$_recall_user_zdotdir/.zshrc"
...
if [[ -z "$__RECALL_HOOKS_LOADED" ]]; then
  __RECALL_HOOKS_LOADED=1
  ...
  _recall_precmd() {
    local _recall_ret=$?
    printf '\e]133;D;%s\e\\' "$_recall_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_recall_urlencode "$PWD")"
