...
- Antigravity: primary session artifacts live under `~/.gemini/antigravity/brain/<session-id>/` on macOS, Linux, and Windows (Windows resolves `~` via `%USERPROFILE%`); Recall also keeps fallback support for legacy app-data chat sessions under macOS `$HOME/Library/Application Support/Antigravity/User/...`, Linux `$HOME/.config/Antigravity/User/...`, and Windows `%APPDATA%\Antigravity\User\...`
