...
- When a newer release exists on GitHub, the sidebar shows an **Update available** action that opens the release page in your default browser.
...
- The release workflow currently builds binaries for **macOS arm64**, **macOS x64**, and **Linux x64**.
...
The release workflow only needs the default `GITHUB_TOKEN` provided by Actions to publish binaries. There are no signing keys or extra secrets to configure.

### How update checks work

Recall does not auto-install updates. On startup (and from **Settings → Check now**) it queries the GitHub Releases API for the latest published release and compares the tag against the running version. When a newer version is available, the sidebar shows an **Update available** action that opens the GitHub release page in your default browser so you can download the new build manually.
