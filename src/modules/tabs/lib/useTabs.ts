...
export function useTabs(initial?: Partial<TerminalTab>) {
   ...
   *   If the path is currently in the preview slot it is promoted in-place.
    *   Use this for programmatic opens (New File dialog, etc.).
  ...
  const openMediaTab = useCallback((path: string, mediaKind: MediaKind, pin = true) => {
    let targetId: number | null = null;
    setTabs((curr) => {
      if (pin) {
        const existing = curr.find(
        ...
        if (existing) {
          targetId = existing.id;
          if ((existing as MediaTab).preview) {
            return curr.map((t) =>
              t.id === existing.id ? { ...t, preview: false } : t,
            );
          }
      ...
      const persistent = curr.find(
        (t) => t.kind === "media" && t.path === path && !(t as MediaTab).preview,
      );
      if (persistent) {
        targetId = persistent.id;
        return curr;
      ...
      const previewIdx = curr.findIndex(
        (t) => t.kind === "media" && (t as MediaTab).preview,
      );
      const id = nextIdRef.current++;
      targetId = id;
      const tab: MediaTab = {
  ...
  const openSessionTab = useCallback((sessionId: string, title?: string | null) => {
    let targetId: number | null = null;
    setTabs((curr) => {
      const existing = curr.find(
      ...
      return [
        ...curr,
        {
          id,
          kind: "session",
          title: title?.trim() || "Session history",
          sessionId,
