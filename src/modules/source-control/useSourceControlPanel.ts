...
type SourceControlPanelState = {
  ...
  branchSnapshot: GitBranchSnapshot | null;
  branchQuery: string;
  branchLocalMatches: GitBranchSnapshot["localBranches"];
  branchRemoteMatches: GitBranchSnapshot["remoteBranches"];
  exactLocalBranch: GitBranchSnapshot["localBranches"][number] | null;
  exactRemoteBranch: GitBranchSnapshot["remoteBranches"][number] | null;
...
export function useSourceControlPanel(
  ...
  const [branchSnapshot, setBranchSnapshot] = useState<GitBranchSnapshot | null>(
    null,
  );
  ...
  const branchResults = useMemo(
    () => searchBranches(branchSnapshot, branchQuery),
    [branchQuery, branchSnapshot],
  );
  ...
  const refreshBranches = useCallback(async () => {
    ...
    const next = await native.gitListBranches(repo.repoRoot);
    setBranchSnapshot(next);
  }, [repo]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void native
      .gitListBranches(repo.repoRoot)
      .then((next) => {
        if (!cancelled) setBranchSnapshot(next);
      })
      .catch((error) => {
        if (!cancelled) setActionError(normalizeError(error));
      });
    return () => {
      cancelled = true;
    };
  }, [isOpen, repo?.repoRoot, status?.branch]);
  ...
  const runBranchAction = useCallback(
    async (
      busyKey: string,
      successMessage: (branch: string) => string,
      action: () => Promise<{ branch: string }>,
    ) => {
      if (!repo || summary.busyAction) return;
      setLocalActionBusy(busyKey);
      setActionMessage(null);
      ...
      try {
        ...
        await summary.refresh({ remote: "never" });
        const nextBranches = await native.gitListBranches(repo.repoRoot);
        setBranchSnapshot(nextBranches);
        setActionMessage(successMessage(result.branch));
      } catch (error) {
        setActionError(normalizeError(error));
      } finally {
        setLocalActionBusy(null);
  ...
  const fetchBranches = useCallback(async () => {
    ...
    setActionMessage(null);
    setActionError(null);
    const result = await summary.runRemoteAction("fetch");
    if (!result.ok) {
    ...
    try {
      ...
      setActionMessage("Fetched remote branches");
    } catch (error) {
      setActionError(normalizeError(error));
    }
  }, [refreshBranches, repo, summary]);
  ...
  const switchLocalBranch = useCallback(
    async (name: string) => {
      ...
      await runBranchAction(
        ...
        () => native.gitSwitchBranch(repo.repoRoot, name),
      );
    },
  ...
  const switchRemoteBranch = useCallback(
    async (name: string) => {
      ...
      await runBranchAction(
        `branch:track:${name}`,
        (branch) => `Switched to ${branch}`,
        () => native.gitSwitchRemoteBranch(repo.repoRoot, name),
      );
    },
    [repo, runBranchAction],
  );
  ...
  const createBranch = useCallback(
    async (name: string) => {
      if (!repo) return;
      const startPoint =
        branchResults.exactRemoteMatch &&
        branchResults.exactRemoteMatch.shortName !== name.trim()
          ? branchResults.exactRemoteMatch.name
          : null;
      await runBranchAction(
        `branch:create:${name}`,
        (branch) => `Created and switched to ${branch}`,
        () => native.gitCreateBranch(repo.repoRoot, name, startPoint),
      );
    },
  ...
  return {
    ...
    branchSnapshot,
    branchQuery,
    branchLocalMatches: branchResults.localBranches,
    branchRemoteMatches: branchResults.remoteBranches,
    exactLocalBranch: branchResults.exactLocalMatch,
    exactRemoteBranch: branchResults.exactRemoteMatch,
    canCreateBranch: branchResults.canCreateBranch,
    selected,
    commitMessage,
