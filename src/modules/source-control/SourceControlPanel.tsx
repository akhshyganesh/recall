...
import type { GitLocalBranch, GitRemoteBranch } from "@/lib/native";
            ...
            <div className="relative shrink-0 space-y-2 border-b border-border/35 bg-card/40 px-2.5 pb-2.5 pt-2.5">
              ...
              >
                <Textarea
                  ...
                  rows={3}
                  className={cn(
                    "min-h-18 resize-none rounded-sm border-0 bg-transparent px-3 pb-7 pt-2.5 text-[12.5px] leading-snug shadow-none placeholder:text-muted-foreground/65 focus:border-0 focus-visible:ring-0",
                  )}
                />
...
function DivergedBanner() {
  return (
    <div className="mx-2 mt-1 flex h-7 items-center gap-1.5 rounded-sm border border-border/60 bg-foreground/4 px-2 text-[10.5px] leading-none text-muted-foreground">
      <HugeiconsIcon
        icon={Alert02Icon}
        size={11}
        strokeWidth={1.9}
...
const EntryRow = memo(function EntryRow({
  ...
  return (
    <div
      ...
      className={cn(
        "group relative flex h-7.5 items-center gap-2 rounded-sm pl-2 pr-2 transition-all duration-100",
        focused
          ? "bg-accent/60"
          : isSelected
    ...
    >
      <span
        className={cn(
          "pointer-events-none absolute inset-y-1 left-0 w-0.5 rounded-sm transition-opacity",
