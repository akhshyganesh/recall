...
export function useFileTree(rootPath: string | null, options?: Options) {
  ...
  const refreshLoadedPaths = useCallback(async () => {
    if (!rootPath || liveRefreshRunningRef.current) return;
    liveRefreshRunningRef.current = true;
    try {
      const loadedPaths = Object.entries(nodesRef.current)
        .filter(([, state]) => state.status === "loaded")
        .map(([path]) => path);
      const uniquePaths = Array.from(new Set([rootPath, ...loadedPaths]));
      await Promise.allSettled(
        uniquePaths.map((path) => fetchChildren(path, true)),
      );
    } finally {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => void refreshLoadedPaths(), 1500);
