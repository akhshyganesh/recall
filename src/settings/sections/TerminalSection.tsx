  ...
  return (
    <div className="flex flex-col gap-4">
      <SettingsCard title="Font">
        ...
        >
          <input
            ...
            className="h-8 w-52 rounded-md border border-border/70 bg-background px-2.5 text-[12px] outline-none transition-colors focus:border-foreground/40"
          />
        </SettingRow>

        <SettingRow title="Font size" description="Terminal text size in pixels.">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="min-w-20 rounded-md p-1">
              {TERMINAL_FONT_SIZES.map((size) => (
                <DropdownMenuItem
                  key={size}
                  onSelect={() => void setTerminalFontSize(size)}
                  className={cn("rounded-md px-3 py-1.5 text-[12px]", size === terminalFontSize && "bg-accent/50")}
                >
                  {size} px
                </DropdownMenuItem>
              ))}
        ...
        >
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                <span>
                  {terminalLetterSpacing > 0 ? `+${terminalLetterSpacing}` : terminalLetterSpacing} px
                </span>
                <HugeiconsIcon icon={ArrowDown01Icon} size={12} strokeWidth={2} className="opacity-70" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="min-w-25 rounded-md p-1">
              {[-4, -3, -2, -1, 0, 1, 2, 3, 4].map((value) => (
                <DropdownMenuItem
                  key={value}
                  onSelect={() => void setTerminalLetterSpacing(value)}
                  className={cn("rounded-md px-3 py-1.5 text-[12px]", value === terminalLetterSpacing && "bg-accent/50")}
                >
      ...
      <SettingsCard title="Performance">
              ...
              <TooltipProvider delayDuration={200}>
                <Tooltip>
                  ...
                  <TooltipContent side="top" className="max-w-65 text-[11px]">
                    ...
                    On some macOS setups, the atlas can corrupt and terminal text
                    becomes unreadable. Disable WebGL as a safe fallback.
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </span>
          }
          description="Hardware-accelerated rendering. Disable if text shows corruption or blank tiles."
        >
        ...
        <SettingRow
          title="Scrollback"
          description="Lines of history kept per terminal pane. Higher values use more RAM."
        >
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
            ...
            <DropdownMenuContent align="end" className="min-w-35 rounded-md p-1">
              {TERMINAL_SCROLLBACK_PRESETS.map((lines) => (
                <DropdownMenuItem
                  key={lines}
                  onSelect={() => void setTerminalScrollback(lines)}
                  className={cn("rounded-md px-3 py-1.5 text-[12px]", lines === terminalScrollback && "bg-accent/50")}
                >
